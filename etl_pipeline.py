...
class ExtractMarketingData(luigi.Task):
    ...
    def run(self):
        ...
        marketing_data = pd.read_csv(self.csv_file)

        # Menyimpan ke CSV
        marketing_data.to_csv(self.output().path, index=False)
    
    def output(self):
        return luigi.LocalTarget("/Users/user/data-eng/data/raw/extract_marketing_data.csv")
...
class ExtractMydramalistData(luigi.Task):
    ...
    def scrape_reviews(self, page):
        ...
        # Mencari semua elemen ulasan
        for review in soup.find_all('div', class_='review'):
            # Mengambil informasi reviewer
            reviewer = review.find('a', class_='text-primary').text.strip() if review.find('a', class_='text-primary') else "Unknown Reviewer"
            ...
            overall_rating = review.find('span', class_='score').text.strip() if review.find('span', class_='score') else "N/A"

            # Mengambil isi review
            review_body_div = review.find('div', class_='review-body')
            review_body = review_body_div.text.strip().replace('\n', ' ') if review_body_div else "No Review Available"

            # Menangani kasus di mana review dipotong dengan "Read More"
    ...
    def run(self):
        ...
        print("Ulasan telah disimpan ke", self.output().path)  # Menampilkan pesan bahwa penyimpanan selesai
...
class ValidateData(luigi.Task):
    ...
    def requires(self):
        return [ExtractAmazonData(), ExtractMarketingData(), ExtractMydramalistData()]
    ...
    def run(self):
        for idx in range(0, 3):
            ...
            # Iterasi untuk setiap kolom
            for col in get_cols:
                # menghitung nilai yang hilang dalam persentase
    ...
    def output(self):
        return [
            ...
            luigi.LocalTarget("/Users/user/data-eng/data/validate/validate_marketing_data.csv"),
...
# Proses Transformasi Marketing Data
class TransformMarketingData(luigi.Task):
    output_file = luigi.Parameter(default="/Users/user/data-eng/data/transform/transform_marketing_data.csv")
    ...
    def run(self):
        ...
        transform_marketing_data = pd.read_csv(self.input()[1].path)  # Corrected variable name
        ...
        available_columns = [col for col in selected_columns if col in transform_marketing_data.columns]
        transform_marketing_data = transform_marketing_data[available_columns]

        # Mengganti nama kolom untuk kemudahan
        transform_marketing_data.rename(columns={
        ...
        # Mapping nilai kolom
        transform_marketing_data.replace({
            'Availability': {
        ...
        if 'Shipping' in transform_marketing_data.columns:
            transform_marketing_data['Shipping'] = (
                transform_marketing_data['Shipping']
        ...
        for col in date_columns:
            if col in transform_marketing_data.columns:  # Check if the column exists
                transform_marketing_data[col] = pd.to_datetime(transform_marketing_data[col], errors='coerce').fillna(method='bfill')
     
        # Menyimpan hasil transformasi ke CSV
        transform_marketing_data.to_csv(self.output().path, index=False)  
...
class TransformMydramalistData(luigi.Task):
    ...
    def run(self):
        ...
        # Inisialisasi daftar untuk menyimpan rating dan review
        overall_ratings = []
        story_ratings = []
        acting_ratings = []
        music_ratings = []
        ...
        for review_body in transform_mydramalist_data['review_body']:
            ...
            overall_ratings.append(overall.group(1) if overall else "N/A")
            story_ratings.append(story.group(1) if story else "N/A")
            acting_ratings.append(acting_cast.group(1) if acting_cast else "N/A")
            music_ratings.append(music.group(1) if music else "N/A")
            rewatch_value_ratings.append(rewatch_value.group(1) if rewatch_value else "N/A")
            
            # Menyaring teks ulasan setelah rating
            review_text = re.split(r'Overall\s*\d+(\.\d+)?|Story\s*\d+(\.\d+)?|Acting/Cast\s*\d+(\.\d+)?|Music\s*\d+(\.\d+)?|Rewatch Value\s*\d+(\.\d+)?', review_body)
            reviews.append(review_text[-1].strip())  # Mengambil bagian terakhir yang merupakan ulasan
        ...
        # Mengonversi kolom rating menjadi tipe numerik, dengan coercing error (mengubah "N/A" menjadi NaN)
        rating_columns = ['overall_rating', 'story_rating', 'acting_rating', 'music_rating', 'rewatch_value']
        for col in rating_columns:
            transform_mydramalist_data[col] = pd.to_numeric(transform_mydramalist_data[col], errors='coerce')
            transform_mydramalist_data[col].fillna(0, inplace=True)  # Mengganti NaN dengan 0
...
class LoadData(luigi.Task):
    ...
    def output(self):
        return [
            ...
            luigi.LocalTarget("/Users/user/data-eng/data/load/load_marketing_data.csv"),
    ...
    def run(self):
        ...
        load_marketing_data.to_sql("MarketingData", con=engine, if_exists="append", index=False)
 ...
 # Memanggil build Luigi untuk menjalankan berbagai task dalam ETL pipeline 
